...
export default function Lesson0103() {
  ...
  const detailedRefrain =
    'อย่างใดอย่างหนึ่ง ที่เป็นอดีต อนาคต และปัจจุบัน ภายในหรือภายนอก หยาบหรือละเอียด เลวหรือประณีต ไกลหรือใกล้ ทั้งหมดเป็นธรรมชาติชนิดหนึ่ง เกิดจากเหตุปัจจัย มาประชุมปรุงแต่งชั่วคราว แล้วแตกสลาย ไม่มีตัวตนเป็นของตนเอง ไม่ควรไปยึดมั่นถือมั่น ว่าสิ่งนั้นเป็นของเรา เป็นเรา เป็นตัวตนของเรา ไม่เที่ยงเกิดดับ';
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-16">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <div className="space-y-4 mb-16">
          ...
          {senses.map((s, i) => (
            <IntrospectionCard key={i}>
              <p className="text-2xl font-bold text-dhamma-blue mb-6">
                {s.subject} {s.object}ไม่เที่ยงเกิดดับ ตัวฉันไม่เที่ยงเกิดดับ
              </p>
              <div className="space-y-4 text-xl text-slate-600 leading-relaxed">
                <p>
                  สิ่งที่มาปรากฏทาง{s.senseType} เป็นเพียงแค่{s.object}
                </p>
                <p>
                  เกิดจากเหตุปัจจัย มาประชุมกันชั่วคราว แล้วแตกสลาย ไม่เที่ยงเกิดดับ{' '}
                  <span className="text-dhamma-gold font-bold ml-2">จริงไหม?—จริง</span>
                </p>
                <p className="pt-4 border-t border-slate-100">
                  ตัวฉันไม่เที่ยงเกิดดับ ตัวฉันเป็นเพียงรูป
                </p>
                <p>
                  เกิดจากเหตุปัจจัย มาประชุมกันชั่วคราว แล้วแตกสลาย ไม่เที่ยงเกิดดับ{' '}
                  <span className="text-dhamma-gold font-bold ml-2">จริงไหม?—จริง</span>
          ...
          {senses.map((s, i) => (
            <IntrospectionCard key={i} highlight={i % 2 === 0}>
              ...
              <div className="space-y-4 text-xl leading-relaxed opacity-90">
                ...
                <p>เกิดจากเหตุปัจจัย มาประชุมกันชั่วคราว แล้วแตกสลาย ไม่เที่ยงเกิดดับ</p>
                <p className="text-xl font-mono tracking-[0.3em] font-bold mt-4 text-center">
        ...
        <div className="space-y-4 mb-16">
          <h4 className="text-center text-slate-400 font-medium mb-8 uppercase tracking-widest">
            ส่วนที่ ๓: พิจารณาความเป็นอนัตตา
          </h4>
          {senses.map((s, i) => (
            <IntrospectionCard key={i}>
              <p className="text-2xl font-bold text-dhamma-blue mb-4">
                {s.subject} {s.object}ไม่เที่ยงเกิดดับ ตัวฉันไม่เที่ยงเกิดดับ
              </p>
            ...
            {aggregates.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                ...
                <p className="text-xl text-slate-700">
                  <span className="font-bold">{item.name}</span> {item.detail && `(${item.detail})`}{' '}
                  ไม่เที่ยง เป็นทุกข์ เป็นอนัตตา ไม่มีตัวตนเป็นของตนเอง ไม่เที่ยงเกิดดับ
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4 mb-16">
          <h4 className="text-center text-slate-400 font-medium mb-8 uppercase tracking-widest">
            ส่วนที่ ๔: พิจารณาขันธ์ 5 โดยละเอียด
          </h4>
          {aggregates.map((item, i) => (
            <IntrospectionCard key={i}>
              ...
              <p className="text-xl text-slate-600 leading-relaxed">{detailedRefrain}</p>
            </IntrospectionCard>
          ))}
        </div>
