...
import { FaChevronDown, FaBars, FaGraduationCap, FaSun } from 'react-icons/fa6';
...
const MobileNav = () => {
  ...
  const menuContent = (
    <>
      ...
      {/* Sidebar */}
      <nav className="fixed top-0 left-0 bottom-0 w-[300px] bg-white h-screen shadow-2xl z-[10001] flex flex-col animate-slide-in-left">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
          ...
          <button onClick={() => setIsOpen(false)} className="text-2xl text-slate-400 p-2">
            <FaTimes />
          </button>
        </div>
        ...
        <div className="flex-grow overflow-y-auto py-4 px-3 custom-scrollbar">
          <ul className="space-y-1.5">
            <li>
              <Link
                href="/"
                className={`flex items-center p-3.5 rounded-2xl text-sm font-medium ${pathname === '/' ? 'bg-blue-50 text-dhamma-blue' : 'text-slate-600'}`}
              >
            ...
            <li>
              ...
              >
                <div className="flex items-center">
                  <FaPray className="mr-3 text-lg opacity-50" /> บทวิปัสสนาภาวนา
                </div>
                <FaChevronDown
                  className={`transition-transform duration-300 ${openDropdown === 'vip' ? 'rotate-180' : ''}`}
                  size={12}
                />
              </button>
              <div
                className={`overflow-hidden transition-all duration-300 ${openDropdown === 'vip' ? 'max-h-[400px] opacity-100 mt-1' : 'max-h-0 opacity-0'}`}
              >
                <ul className="ml-10 space-y-1 border-l-2 border-blue-100">
                  {vipassanaLessons.map((l) => (
                    <li key={l.href}>
                      <Link
                        href={l.href}
                        className="block p-2.5 text-sm text-slate-500 hover:text-dhamma-blue"
                      >
                        {l.label}
                      </Link>
                    </li>
            ...
            <li>
              ...
              >
                <div className="flex items-center">
                  <FaGraduationCap className="mr-3 text-lg opacity-50" /> หลักสูตรอนาคามีผล
                </div>
                <FaChevronDown
                  className={`transition-transform duration-300 ${openDropdown === 'ana' ? 'rotate-180' : ''}`}
                  size={12}
                />
              </button>
              <div
                className={`overflow-hidden transition-all duration-300 ${openDropdown === 'ana' ? 'max-h-[600px] opacity-100 mt-1' : 'max-h-0 opacity-0'}`}
              >
                <ul className="ml-10 space-y-1 border-l-2 border-amber-100">
                  {anagamiLessons.map((l) => (
                    <li key={l.href}>
                      <Link
                        href={l.href}
                        className="block p-2.5 text-sm text-slate-500 hover:text-amber-700"
                      >
                        {l.label}
                      </Link>
                    </li>
            ...
            <li>
              <Link
                href="/extra"
                className="flex items-center p-3.5 text-slate-600 text-sm font-medium"
              >
  ...
  return (
    <>
      <button
        ...
        className="text-2xl text-slate-700 p-2 hover:bg-slate-100 rounded-xl transition-colors"
      >
        <FaBars />
      </button>
